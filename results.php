...
printResultsTable($year, $oneAct);


function printFesprintResultsTabletivalInfo($festivalName, $year, $oneAct)
{
    ...
    $resultsArray = buildResultsArray ( $oneActStr, $year, $oneAct );
    printResultsTable ( $resultsArray );
}

function buildResultsArray($oneActStr, $year, $oneAct) {
    $conn = getDbConnection ();
    $resultsArray = array (
        array ()
    );
    $groupAndPlayList = getListOfGroupsAndPlays ( $conn, $oneActStr, $year, $oneAct );
    if ($groupAndPlayList->num_rows > 0) {
        ...
        while ( $row = $groupAndPlayList->fetch_assoc () ) {
            ...
            $resultsArray [$rowNum] ['numWins'] = getNumPlacings ( $conn, $groupName, $oneActStr, $year, $oneAct, $play, 'FIRST' );
            $resultsArray [$rowNum] ['numSeconds'] = getNumPlacings ( $conn, $groupName, $oneActStr, $year, $oneAct, $play, 'SECOND' );
            $resultsArray [$rowNum] ['numThirds'] = getNumPlacings ( $conn, $groupName, $oneActStr, $year, $oneAct, $play, 'THIRD' );
        ...
        echo "No results recorded yet for $oneActStr competition";
    }
    $conn->close ();
}


function printResultsTable($resultsArray) {
...
function getNumFestivalsEntered($conn, $groupName, $year, $oneAct, $play) {
    $formattedGroupName = mysql_real_escape_string ( $groupName );
    if ($oneAct == "") {
        ...
        $formattedPlayName = mysql_real_escape_string ( $play );
        $sql = "SELECT count(*) as num_entries FROM " . $oneAct . "NIGHTS_" . $year . " where " . $oneAct . "NIGHTS_" . $year . ".GROUP='$formattedGroupName'
...
function getNumPlacings($conn, $groupName, $oneActStr, $year, $oneAct, $play, $place) {
    $formattedGroupName = mysql_real_escape_string ( $groupName );
    $groupNameColumn = $oneActStr . "_" . $place;
    ...
    if ($oneAct == "") {
        ...
        $formattedPlayName = mysql_real_escape_string ( $play );
        $playNameColumn = $oneActStr . "_" . $place . "_PLAY";
...
function getListOfGroupsAndPlays($conn, $oneActStr, $year, $oneAct) {
    if ($oneAct == "") {
        $sql = "SELECT NAME, PLAY_2016 FROM DRAMA_GROUP where LEVEL='$oneActStr'";
    } else {
        $sql = "SELECT DISTINCT `GROUP`, PLAY FROM " . $oneAct . "NIGHTS_" . $year . " JOIN " . $oneAct . "DRAMA_GROUP ON " . $oneAct . "DRAMA_GROUP.NAME=" . $oneAct . "NIGHTS_" . $year . ".GROUP WHERE " . $oneAct . "DRAMA_GROUP.LEVEL='" . $oneActStr . "'";
    }
    $result = $conn->query ( $sql );
    return $result;
}
...
function printCss() {
    $filename = 'http://adci.ie/wp-content/plugins/php-code-for-posts/results.css';
